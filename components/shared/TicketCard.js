...
const Card = ({ event, hasOrderLink, hidePrice, userId }) => {
  ...
  return (
    <div className="group relative flex min-h-[280px] w-full max-w-[300px] flex-col overflow-hidden rounded-xl bg-white shadow-md transition-all hover:shadow-lg md:min-h-[438px]">
      <Link 
        href={`/ticket?event=${event.eventURL}&id=${event.ticket_id}&type=org`}
      ...
      > 
       <div className="flex gap-2">
        ...
        <div className="flex-between w-full">
        <Link href={`/ticket?event=${event.eventURL}&id=${event.ticket_id}&type=org`}>
